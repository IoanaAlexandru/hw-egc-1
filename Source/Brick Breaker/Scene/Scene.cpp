...
const float Scene::BRICK_PANEL_WIDTH_RATIO = 0.75,
            ...
            Scene::WALL_THICKNESS_RATIO = 0.05,
...
void Scene::Init()
{
	...
	float width = 1280, height = 710;  // TODO how to get these?

	float wall_thickness = min(width, height) * WALL_THICKNESS_RATIO;
	float brick_width = width * BRICK_PANEL_WIDTH_RATIO / BRICKS_PER_ROW * (1 - BRICK_DISTANCE_RATIO),
	      brick_height = height * BRICK_PANEL_HEIGHT_RATIO / BRICK_ROWS * (1 - BRICK_DISTANCE_RATIO);
	float brick_distance_x = (width * BRICK_PANEL_WIDTH_RATIO - BRICKS_PER_ROW * brick_width) / (BRICKS_PER_ROW - 1),
		  brick_distance_y = (height * BRICK_PANEL_HEIGHT_RATIO - BRICK_ROWS * brick_height) / (BRICK_ROWS - 1);
	float x_offset = width * (1 - BRICK_PANEL_WIDTH_RATIO) / 2,
		y_offset = height - (wall_thickness + brick_height);
